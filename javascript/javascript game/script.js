...
const gameModeConfig = {
    ...
    'Endless': {
        ...
        enemyStats: {
            base: { health: 500, damage: 45, speed: { min: 0.4, max: 0.8 } },
            fast: { health: 300, damage: 30, speed: { min: 0.8, max: 1.5 } },
            tank: { health: 2500, damage: 70, speed: { min: 0.3, max: 0.6 } },
            boss: { health: 1000000, damage: 2000, speed: { min: 0.2, max: 0.2 } }
        },
        towerStats: {
        ...
        initialEnemyInterval: 500,
        spawnTiers: [
            { scoreThreshold: 30000, minInterval: 10, decreaseAmount: 1 },
            { scoreThreshold: 8500, minInterval: 30, decreaseAmount: 2 },
            { scoreThreshold: 1200, minInterval: 120, decreaseAmount: 10 },
            { scoreThreshold: 0, minInterval: 150, decreaseAmount: 20 }
        ],
...
window.addEventListener('keydown', function(e) {
    ...
    if (keyToTower[e.key]) {
        const towerIndex = keyToTower[e.key];
        
        const isMinigunner = towerIndex === 5;
        const isMinigunnerLocked = isMinigunner && !beatenModes.has('Normal');
        
        const isAccel = towerIndex === 7;
        const isAccelLocked = isAccel && !beatenModes.has('Hard');

        if (isMinigunnerLocked) {
            floatingMessages.push(new floatingMessage('Beat Normal to unlock!', canvas.width / 2, canvas.height / 2, 20, 'red'));
        } else if (isAccelLocked) {
            floatingMessages.push(new floatingMessage('Beat Hard to unlock!', canvas.width / 2, canvas.height / 2, 20, 'red'));
        } else {
            chosenTower = towerIndex;
            sellMode = false;
        }
    }
